...
3. **React companion app** (`companion-app/`) — Vite + TS + Tailwind. Everything centers on **one surface**, the **plan grid** (`RoleColumnsGrid`): five role columns — Drums · Bass · Vocals · Other · Song ("song" == the recommender's "mix") — each stacking the DJ's **queued plan picks on top** and **recommendations below**. The same grid renders in two modes. Three views: **Booth** (live performance: MasterStrip + TwoDeckStrip stacked-stem waveforms (one panel per deck, A/B) + Crossfader + BoothColumnHeaders + 8-column 2-deck SceneGrid mirror + CueStrip + the plan grid in `mode="live"` — recs tail what's playing in Ableton, each card has ⤒A/⤒B to load that pick onto a deck), **Set** (the plan grid in `mode="plan"` — recs scored against the rest of the plan + the plan's journey, no deck-load), **Pipeline** (ingest/processing status; also library inventory via the Done-column filter). A **Set IS its plan**: a JSON `plan` column on `sets`, shape `{role: [track_id,...]}`; one `is_active` at a time. Build it by queuing per-role recs (＋) or via **⌘K** (which appends to the Song column, server-side merge). Hybrid **⌘K** palette combines fuzzy artist/title search with CLAP vibe results. The unified rec brain (`recommender/scoring.py` + `journey.py` + `structure.py`) powers all recs — trend-aware "journey" vibe, halftime-aware BPM, kick-density/presence/timbre, transition fit — scoped per role and re-scored against context (the active stem combo in Booth, the rest of the plan in Set). See [`docs/dj_ux_flow.md`](docs/dj_ux_flow.md) for the full surfaces map.
...
| "let's plan a set" / "make a set" | Open the companion app → **Set** view → create a set → queue picks per role from the recs (＋ on a card) or via ⌘K (appends to the Song column). The plan IS the set (JSON `plan` on `sets`); recs are scored against the rest of the plan + the plan's journey. The same grid drives the Booth in `mode="live"`. |
| "recs seem off" / "tune the rec scoring" | Scoring lives in the unified brain — [`src/dance/recommender/scoring.py`](src/dance/recommender/scoring.py) (+ [`journey.py`](src/dance/recommender/journey.py) for trend/vibe, [`structure.py`](src/dance/recommender/structure.py) for transition fit). Run a real-set check before changing. (`recommender/tail.py` + `/sets/{id}/tail-recs` are legacy — not used by the UI.) |
